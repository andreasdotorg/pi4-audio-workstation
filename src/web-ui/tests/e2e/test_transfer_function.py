...
- Degraded-mode banner is visible when PCM sources are missing
  (local-demo only has 'monitor', not 'capture-usb')
...
class TestTfDegradedMode:
    """When PCM sources are missing, the fallback must be visible.

    Local-demo only has 'monitor' (port 9090), not 'capture-usb'.
    The server falls back to mock mode with a mock_fallback flag.
    The UI must show a degraded-mode banner so the user knows
    they're seeing synthetic data.
    """

    def test_mock_fallback_banner_visible(self, tf_page):
        """The degraded-mode banner is visible when PCM sources are missing.

        This verifies the 'visible, not silent' requirement from F-270.
        """
        banner = tf_page.locator("#tf-mock-fallback-banner")
        try:
            tf_page.wait_for_function(
                "(() => {"
                "  var el = document.getElementById('tf-mock-fallback-banner');"
                "  return el && el.style.display !== 'none';"
                "})()",
                timeout=DATA_TIMEOUT_MS,
            )
        except Exception:
            # The banner may not show if PI4AUDIO_PCM_SOURCES is configured
            # with both sources.  In standard local-demo it should show.
            display = banner.evaluate("el => el.style.display")
            if display == "none":
                pytest.skip(
                    "Mock fallback banner not shown — PCM sources may be "
