...
```sh
scp scripts/nixos-smoke-test.sh ela@192.168.178.185:/tmp/
ssh ela@192.168.178.185 sudo bash /tmp/nixos-smoke-test.sh
```
...
nix run nixpkgs#nixos-rebuild -- switch \
    --flake .#mugge-deploy \
    --target-host ela@192.168.178.185 \
    --sudo
```
