...
**Status:** IMPLEMENT — core crossover engine done (unit tests pass), but 4 integration defects block Gate 1: F-188 (pcm-bridge 4ch mismatch), F-189 (graph tab 4ch), F-190 (meters 4ch), F-191 (GM routing 4ch). All are hardcoded 4ch assumptions that break on 3-way (6ch) topology. Reverted from REVIEW to IMPLEMENT (session 5, AD finding F6).
