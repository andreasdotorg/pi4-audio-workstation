...
**Status:** REVIEW — all advisory approvals collected (session 5). Architect APPROVED, AE APPROVED, UX APPROVED. AE note: per-channel sweep sequencing must be documented as mandatory (one channel at a time, not simultaneous). Awaiting owner acceptance + Pi deployment (Gate 2).
