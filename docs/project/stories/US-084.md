...
**Status:** IMPLEMENT 10/13 (Phase 1 DONE: crate extracted, 45/45 tests. Phase 2 DONE: pcm-bridge stripped, 42/42 tests. Phase 3 DONE: local-demo.sh + flake.nix. Local verification DONE 2026-03-29: web UI wiring, signal-gen mono, GM-managed linking all verified against local PW. Remaining: systemd templates (Pi), 24 meters on Pi, owner visual acceptance.)
...
(D-047 snapshot rate). Each instance registers with a well-known node name;
GM manages the links per D-039 (GM is sole link manager).
...
- [x] Each instance registers with a well-known node name and GM manages links to correct PW nodes per D-039 (amended from "self-links" per Architect confirmation 2026-03-29)
...
- [ ] Architect review (binary separation, GM-managed link design)
