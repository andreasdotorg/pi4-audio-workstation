...
| TK-030 | Lab notes: add script/config/result cross-references | technical-writer | open | standalone | owner request (documentation linkage) | For each test in lab notes (US-001, US-002, US-003, US-028): add relative paths to the script used, config file, and result file. **Unblocked:** TK-024 done (canonical scripts identified). |
| TK-031 | Repo layout section in top-level README | technical-writer | open | standalone | owner request (documentation linkage) | Add or update repo layout section describing `scripts/`, `configs/`, `results/`, `data/`, `docs/` directory purposes. **Unblocked:** TK-026-029 all done (individual READMEs created). |
...
- Tasks TK-001 through TK-034 are active (done in place: TK-002, TK-015, TK-016, TK-023 (FAIL), TK-024, TK-025, TK-026, TK-027, TK-028, TK-029, TK-033, TK-034). TK-100+ are completed (numbering gap is intentional for clarity).
- TK-024 through TK-034 are the reproducibility/repo-structure audit (owner request). TK-024 done (QE audit). TK-025 done (dead code cleanup). TK-026-029 done (all four READMEs created). TK-033 done (T3e raw data). TK-034 done (number fix). **Remaining:** TK-030 (lab notes cross-refs, now unblocked), TK-031 (repo layout README, now unblocked), TK-032 (T3e script).
