...
| TK-016 | Reaper smoke test: connect via RustDesk, launch Reaper, verify it runs with a project loaded | change-manager | open | US-030 | team-lead directive (UAT gap) | **F-012:** Reaper causes hard kernel lockup on PREEMPT_RT. Works on stock PREEMPT (Test B PASS, 90s clean). `chrt -o 0` also crashes on RT. Per D-015: proceed on stock PREEMPT. TK-016 is unblocked on stock kernel. |
...
- Tasks TK-001 through TK-022 are active. TK-023 is done (FAIL). TK-100+ are completed (numbering gap is intentional for clarity).
