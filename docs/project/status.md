...
| US-050 | IMPLEMENT | 4/6 | **in-progress — E2E-tier 3/8 done.** CI-tier DONE (4/4). **E2E progress:** EH-1 DONE (85 lines, 9 tests), EH-3 DONE (113 lines), EH-4 DONE (230 lines). EH-2 in-progress (worker-mock-backend). EH-5 ready (EH-4 done). EH-6/7/8 blocked. Track B complete. Critical path: EH-2->EH-5->EH-6->EH-7->EH-8. (5) E2E harness: 3/8 tasks done. (6) E2E test suite: blocked on EH-6. |
...
- **TK-151** (**DONE — S-001 Steps 3-9 ALL PASS**): pcm-bridge runtime validated on Pi. AD-F006 build chain + runtime both proven. Step 6 PASS (stream connects), Step 7 PASS (440Hz audio capture), Step 8 PASS (TCP streaming, 11.9MB), Step 9 PASS (graceful shutdown). Fixes: type `77e7710`, routing `d7ae503`, SPA format `89c8500`. **Residual TK-236 (non-blocking):** WirePlumber auto-link issue (1 port instead of 8, manual pw-link workaround). Step 10 (extended stability) deferred. **Unblocked:** SG-10, SB-7b, all Pi-gated subtasks.
...
- **US-052** (Phase: **IMPLEMENT** — 9/12 subtasks done, **all local Rust code complete, TK-151 DONE**): Real-time signal generator in Rust (6,183 lines, 8 modules). D-037 APPROVED (TK-234 DONE). SG-1 through SG-9 ALL DONE. **SG-10 IN PROGRESS:** Cargo.lock resolved (`f56f0f5`), signal-gen build #2 launched on Pi with 0.8.0 fixes. SG-11/12 blocked on SG-10. Critical path: SG-10->SG-11->SG-12. **TK-151 DONE** — pcm-bridge validated, Rust-on-Pi build chain + runtime proven. Supersedes TK-229. Blocks US-053, US-047, US-012.
- **US-050** (Phase: **IMPLEMENT** — E2E-tier 3/8 done, Track B complete): Measurement pipeline test harness. **CI-tier DONE.** **E2E-tier progress:** EH-1 DONE (IR export, 85 lines, 9 tests), EH-3 DONE (CDSP config, 113 lines), EH-4 DONE (process manager, 230 lines, integration tested). **EH-2 IN PROGRESS** (worker-mock-backend). EH-5 READY (deps met). EH-6/7/8 blocked. Track B (worker-test-tool) complete + idle. Remaining critical path: EH-2->EH-5->EH-6->EH-7->EH-8. EH-5+ require Pi. Blocks US-047.
