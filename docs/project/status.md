...
| US-050 | IMPLEMENT | 5/6 | **in-progress — E2E-tier 8/8 ALL DONE.** CI-tier DONE (4/4). **E2E-tier ALL DONE:** EH-1-8 complete. (5) E2E harness: DONE (8/8 tasks). (6) E2E test suite: code written, **pending Pi execution** (tests require PipeWire). Ready to advance to TEST once E2E tests pass on Pi. |
...
- **TK-151** (**DONE — S-001 Steps 3-9 ALL PASS**): pcm-bridge runtime validated on Pi. AD-F006 build chain + runtime both proven. **TK-236 DONE** (`7f987c1`): `audio.channels` property fix committed, pending Pi rebuild. Step 10 (extended stability) deferred. **worker-pcm-bridge now idle.**
...
- **US-050** (Phase: **IMPLEMENT** — E2E-tier 8/8 ALL DONE, pending Pi execution): Measurement pipeline test harness. **CI-tier DONE.** **E2E-tier ALL DONE:** EH-1 (IR export), EH-2 (room sim config), EH-3 (CDSP config), EH-4 (process manager), EH-5 (PW wiring), EH-6 (pytest fixture), EH-7 (signal gen integration tests), EH-8 (measurement workflow test). All code written. **Tests require Pi (PipeWire) for execution.** Pending CM commit for EH-7+EH-8. Advance to TEST once E2E tests pass on Pi. worker-mock-backend + worker-test-tool now idle. Blocks US-047.
