...
| US-071 | **REVIEW** | **8/9** | **Owner review remaining.** AE review DONE (approved, F-066 MUST-FIX applied). Security review DONE (approved). AC1 SETUP-MANUAL 6/8 (web UI 7-tab + test plan still open). AC2-AC7 all DONE. F-066 corrected (Mult session-only). Checkboxes updated. **DoD 8/9.** Remaining: #9 owner review only. |
...
- **F-064** (PARTIAL FIX `e77d863`, deployed): Collector timeouts reduced + Nice=10→Nice=0. Web-UI intermittently reachable — partial fix only. **Real fix:** Three-tool RT architecture redesign (eliminates Python relay). **Architect + AE unified design COMPLETE (2026-03-22):** Three binaries sharing `audio-common` crate: (1) `level-bridge` — always-on systemd, 2 PW streams (convolver in+out), WS JSON 10Hz :9100; (2) `pcm-bridge` — on-demand taps only, GM-managed lifecycle, WS binary PCM :9200+; (3) `signal-gen` — unchanged, Python proxy stays for D-009 safety. 5-phase plan: P1 F-064 (done), P2 level-bridge + pcm-bridge WS + shared crate (2-3d), P3 reverse proxy + remove Python relay (1d), P4 GM on-demand taps start_tap/stop_tap RPC (3-5d), P5 measurement session integration (2-3d). Constraints: `node.passive=true` on all taps, per-channel selection in browser JS, GM manages tap lifecycle, web-UI orchestrates measurement workflow, zero data coupling between RT services. **Strategic decisions (Q6-Q8, all confirmed by architect + AE + AD + QE + security):** No fourth recording binary (signal-gen `playrec` handles measurement capture with same-process sample sync — decisive per AE/AD). Web-UI Python owns orchestration (no RT-critical steps outside signal-gen). No consolidation planned — **hard security constraint:** safety-critical tier (GM watchdog, signal-gen hard cap) must NEVER share binary with data-plane tier (pcm-bridge, level-bridge). Same-tier merges permitted if triggers fire but not warranted today. **Caddy** as reverse proxy (not nginx) — auto TLS, simple WS config, solves F-037 auth gap. Web-UI stays Python permanently (control plane = Python, data plane = Rust). **Phasing revised (architect adjudication of AD challenge):** Phase 2a (HIGH PRIORITY): fix collectors to use Rust service RPC instead of subprocess — directly fixes F-061/F-063/F-064. Phase 2b: add tungstenite WS to pcm-bridge. Phase 3: Caddy reverse proxy + remove Python PCM relay (optimization, becomes important with on-demand taps). Phase 4: GM on-demand taps. Phase 5: measurement session integration. **All design questions Q1-Q8 CLOSED.** Tracked under US-060/US-066.
