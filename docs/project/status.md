...
- **US-052** (Phase: **IMPLEMENT** — BUG-SG12-4 + BUG-SG12-5 both CODE-COMPLETE, pending CM Pi build + runtime verification): Real-time signal generator in Rust (6,183 lines, 8 modules). D-037 APPROVED. SG-1 through SG-11 ALL COMMITTED. **SG-12 Pi acceptance:** T1 FAIL (42ms), T2-T4 INVALID (no PW ports), T5 FAIL (BUG-SG12-4), T7 INVALID. **ALL KNOWN BUGS CODE-COMPLETE:** BUG-SG12-5 fix delivered (`build_audio_format()` + `media.class` + 6 tests), BUG-SG12-4 Fix B delivered (playrec capture check + 3 tests), BUG-SG12-1/2/3 FIXED (committed). **Next:** CM builds on Pi, deploys, reruns SG-12 audio tests (T2-T4, T5, T7). Commits: `e74ad99`, `dcbcf4d`, `9c79856`. Unblocks US-053, US-047, US-012 (when verified).
