...
| F-083 | High | Open | No spectrum on dashboard. Commit `197c97c` DRIVER flag fix INSUFFICIENT — local PW 1.4.10 testing confirms ENOTSUP persists. Root cause: premature `set_active(true)` before links exist (format negotiation fails without peers). Three bugs in pcm-bridge + signal-gen stream activation. Rework needed. |
