...
- **US-052** (in-progress, SG-1 worker dispatched 2026-03-15): Real-time signal generator in Rust. D-037 APPROVED (TK-234 DONE). **SG-1 through SG-12 filed.** SG-1 in progress (Rust scaffold). 3 parallel tracks: Rust A (generators), Rust B (safety/RPC/capture), Python C (client). Critical path: SG-1->SG-4->SG-5->SG-6->SG-7->SG-8->SG-10->SG-11->SG-12. SG-1 through SG-5 + SG-9 pure software. TK-151 nix build in progress (gates Pi integration tests). Supersedes TK-229. Blocks US-053, US-047, US-012.
- **US-050** (in-progress, worker-mock-backend dispatched 2026-03-15): Measurement pipeline mock backend. TK-165 foundation done (MockSoundDevice + MockCamillaClient). Remaining: simulated room IR wiring, e2e test scenario (sweep -> record -> deconvolve -> verify). ~200 lines per architect. Blocks US-047.
- **US-053** (in-progress, TT-1 worker dispatched 2026-03-15): Manual test tool page. TT-1 HTML/CSS scaffold in progress. UX spec complete (`test-tool-page.md`).
