...
| US-052 | IMPLEMENT | 3/6 | 11/12 subtasks done. **SG-12: ALL FIXES CODE-COMPLETE, pending CM build #7 + retest.** BUG-SG12-5 (media.class + format params) + TK-236 (`audio.position` + position array, 10 tests) + BUG-SG12-4 Fix B (capture check, 3 tests) all delivered. **CAVEAT:** `SPA_TYPE_Array = 14` unverified on Pi. T1 FAIL (42ms), T2-T7 pending retest after build #7. BUG-SG12-1/2/3 FIXED. (1) Design doc: DONE. (2) Binary on Pi: DONE. (3) Integration test: DONE. (4) RT safety audit: PASS. (5) Stability: pending retest. (6) Lab note: not yet. |
...
- **TK-151** (**DONE — S-001 Steps 3-9 ALL PASS**): pcm-bridge runtime validated on Pi. AD-F006 build chain + runtime both proven. **TK-236 CODE-COMPLETE (HIGH).** Fix: `audio.position` property in stream props + SPA format pod. Playback: AUX0-AUX7 (8 output ports matching loopback sink). Capture: MONO. 10 regression tests. Pending CM build #7 on Pi. pcm-bridge capture mode may need same fix (future item).
...
- **US-052** (Phase: **IMPLEMENT** — ALL FIXES CODE-COMPLETE, pending CM build #7 + Pi retest): Real-time signal generator in Rust (6,183 lines, 8 modules). D-037 APPROVED. SG-1 through SG-11 ALL COMMITTED. **SG-12: 3 fixes delivered, pending build #7:** (1) BUG-SG12-5: `media.class` + SPA format params (6 tests), (2) TK-236: `audio.position` + position array in SPA pod — playback AUX0-AUX7 (8 ports), capture MONO (10 tests), (3) BUG-SG12-4 Fix B: `capture_connected` pre-check (3 tests). **CAVEAT:** `SPA_TYPE_Array = 14` unverified — CM must check Pi headers. BUG-SG12-1/2/3 FIXED (committed). **Next:** CM builds #7, verifies `pw-link -l` shows 8 output ports linked to loopback sink, reruns T2-T7. Unblocks US-053, US-047, US-012 (when verified).
