...
| US-052 | IMPLEMENT | 2/6 | 11/12 subtasks code-complete. **SG-11 code-complete, pending CM commit.** SG-12 ready (assigned worker-mock-backend). (1) Design doc: DONE. (2) Binary on Pi: **DONE** (build #4 PASS). (3) Integration test: code written, local tests pass, **pending CM commit + Pi validation** (SG-11). (4) RT safety audit: not yet (SG-12). (5) 5-min stability: not yet (SG-12). (6) Lab note: not yet. |
...
- **TK-151** (**DONE — S-001 Steps 3-9 ALL PASS**): pcm-bridge runtime validated on Pi. AD-F006 build chain + runtime both proven. **TK-236 OPEN** — both fix attempts failed on Pi (`7f987c1` audio.channels, `f9c6814` SPA format pod). Root cause: PipeWire stream API doesn't support multi-port via format params. Needs architect investigation (JACK API or filter-chain). Non-blocking (pw-link workaround works). Step 10 (extended stability) deferred.
...
- **US-052** (Phase: **IMPLEMENT** — 11/12 subtasks code-complete, **SG-11 code-complete pending commit**): Real-time signal generator in Rust (6,183 lines, 8 modules). D-037 APPROVED (TK-234 DONE). SG-1 through SG-10 committed. **SG-11 code-complete:** daemon integration written (session.py, app.py, signal_gen_client.py), local tests pass, **pending CM commit**. **SG-12 READY** (assigned worker-mock-backend, blocked on SG-11 commit): acceptance testing on Pi. Critical path: SG-11 commit -> SG-12. **TK-151 DONE** — pcm-bridge validated, Rust-on-Pi build chain + runtime proven. Blocks US-053, US-047, US-012.
...
- **US-053** (Phase: **IMPLEMENT** — TT-1 scaffold done, US-052 backend NOW AVAILABLE): Manual test tool page. TT-1 HTML/CSS scaffold DONE (`f3fcfa2`). UX spec complete (`test-tool-page.md`). Signal-gen RPC on Pi (TCP 4001). **TT-2 assigned to worker-signal-gen** — wire test tool page to signal-gen RPC (new `app/test_tool/routes.py` + `static/js/test.js`). US-051 status bar frame code committed (Pi validation in parallel via SB-7 Phase B).
