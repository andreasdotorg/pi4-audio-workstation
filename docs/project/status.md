...
| US-052 | IMPLEMENT | 3/6 | 11/12 subtasks done. **SG-11 committed (`5fe5bf6`). SG-12 in progress (worker-mock-backend).** (1) Design doc: DONE. (2) Binary on Pi: **DONE** (build #4 PASS). (3) Integration test: **DONE** (`5fe5bf6`, committed + tests pass). (4) RT safety audit: not yet (SG-12). (5) 5-min stability: not yet (SG-12). (6) Lab note: not yet. |
| US-053 | IMPLEMENT | 1/6 | TT-1 scaffold + **TT-2 code-complete** (322-line `test.js`, pending CM commit). Signal-gen RPC wiring done: WS lifecycle, all controls, D-009 hard cap, TK-203 pre-action confirm, emergency stop. 117/117 tests pass. Remaining: spectrum, SPL, UMIK-1 hot-plug (separate tasks). (1) Page implemented: code-complete, pending commit + Pi validation. (2)-(6) not yet. |
...
- **US-052** (Phase: **IMPLEMENT** — 11/12 subtasks done, **SG-11 committed, SG-12 in progress**): Real-time signal generator in Rust (6,183 lines, 8 modules). D-037 APPROVED (TK-234 DONE). SG-1 through SG-11 ALL COMMITTED. **SG-11 committed** (`5fe5bf6`): SignalGenClient wired as sd_override, `/ws/siggen` proxy. **SG-12 IN PROGRESS** (worker-mock-backend): acceptance testing on Pi. Critical path: SG-12. **TK-151 DONE** — pcm-bridge validated, Rust-on-Pi build chain + runtime proven. Blocks US-053, US-047, US-012.
...
- **US-053** (Phase: **IMPLEMENT** — TT-2 code-complete, pending CM commit): Manual test tool page. TT-1 scaffold DONE (`f3fcfa2`). **TT-2 CODE-COMPLETE** (worker-test-tool): `test.js` rewritten to 322 lines + `style.css` +16 lines. Full signal-gen RPC wiring via `/ws/siggen`. D-009 hard cap, TK-203 pre-action confirm, emergency stop, state feedback. 117/117 tests pass. **Pending CM commit.** Remaining items (out of TT-2 scope): spectrum analyzer, SPL computation, UMIK-1 hot-plug — separate tasks needed.
